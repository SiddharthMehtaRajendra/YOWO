...
class Yolo_dataset(Dataset):
    def __init__(self, lable_path, cfg):
        ...
        with open(lable_path, 'r') as file:
            self.truth = file.readlines()

    def __len__(self):
        return len(self.truth)

    def __getitem__(self, index):
        base_path = self.cfg.dataset_dir
        imgpath = self.truth[index].rstrip()
        ...
        bboxes = np.array(np.loadtxt(labpath), dtype=np.float)
        bboxes = np.roll(bboxes, -1)
        img_path = os.path.join(base_path, 'rgb-images', im_split[0], im_split[1] ,'{:05d}.jpg'.format(im_ind))
